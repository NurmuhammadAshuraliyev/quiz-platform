// Professional Email Service with real functionality
...
export class EmailService {
  ...
  static async sendContactMessage(data: ContactFormData): Promise<{
    success: boolean;
    messageId?: string;
    error?: string;
  }> {
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="highlight">
              <p><strong>Mavzu:</strong> ${this.getSubjectText(
                data.subject
              )}</p>
              <p><strong>Yuborilgan vaqt:</strong> ${new Date(
                data.timestamp
              ).toLocaleString("uz-UZ")}</p>
              <p><strong>Xabar ID:</strong> #${Date.now()
                .toString()
                .slice(-6)}</p>
            </div>
            ...
            <a href="mailto:${this.ADMIN_EMAIL}?subject=Qo'shimcha savol - ${
      data.name
    }" class="button">
  ...
  private static getAdminNotificationTemplate(data: EmailData): EmailTemplate {
    const subject = `🔔 Yangi xabar: ${this.getSubjectText(data.subject)} - ${
      data.name
    }`;
    const body = `
      <!DOCTYPE html>
      ...
      <head>
        ...
        <style>
          ...
          .priority-${
            data.priority
          } { border-left: 5px solid ${this.getPriorityColor(data.priority)}; }
      ...
      <body>
        <div class="container">
          ...
          <div class="content priority-${data.priority}">
            ...
            <div class="actions">
              <a href="mailto:${data.email}?subject=Re: ${this.getSubjectText(
      data.subject
  ...
  static getContactMessages(): EmailData[] {
  ...
  static getMessagesByPriority(priority: EmailData["priority"]): EmailData[] {
    return this.getContactMessages().filter((msg) => msg.priority === priority);
  }
